...
class OnboardingViewModel: ObservableObject {
    ...
    func getPageTitle(for index: Int) -> String {
        return pages[safe: index]?.title ?? ""
    }
    
    func getPageIcon(for index: Int) -> String {
        return pages[safe: index]?.iconName ?? "questionmark"
    }
    
    func getCompletionDate() -> Date? {
        return userDefaults.object(forKey: UserDefaultsKeys.onboardingCompletedDate) as? Date
    }
